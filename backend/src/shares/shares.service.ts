...
export class SharesService {
  ...
  async listMyShares(ownerId: string) {
    return this.shareModel
      .find({ ownerId: new Types.ObjectId(ownerId), isRevoked: { $ne: true } })
      .lean();
  }
  ...
  async listSharedWithMe(userId: string, userEmail: string | undefined) {
    ...
    const items = await this.driveItemsService.model
      .find({ _id: { $in: itemIds }, isDeleted: false })
      .lean();
  ...
  async listSharedFolderChildren(
    ...
    return this.driveItemsService.model
      .find({ parentId: objectId, isDeleted: false })
      .lean();
  }
