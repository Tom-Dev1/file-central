...
describe('PermissionsService', () => {
      ...
      find: jest.fn((filter: any) =>
        createQuery(async () => {
          ...
          return shares.filter((s) => {
            ...
            const matchesUser =
              (s.sharedWithUserId && s.sharedWithUserId.equals(userB)) ||
              s.sharedWithEmail === userB_email;
  ...
  it('9. requireAccess throws ForbiddenException when permission is insufficient', async () => {
    ...
    await expect(
      service.requireAccess(userB.toString(), userB_email, file._id, SharePermission.EDIT),
  ...
  it('10. requireAccess (and getAccess) throws NotFoundException for a soft-deleted item', async () => {
    ...
    await expect(
      service.getAccess(userA.toString(), undefined, file._id),
