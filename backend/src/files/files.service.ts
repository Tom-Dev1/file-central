...
export class FilesService {
  ...
  /**
   * Upload flow (see design doc §8.1 for why the ordering/streaming matters):
  ...
  async upload(
    ...
    const objectKey = `users/${ownerId}/files/${randomUUID()}-${file.originalname}`;
    ...
    try {
      ...
      try {
        ...
        this.logger.warn(
          `Mongo save failed after MinIO upload, rolling back object ${objectKey}`,
        );
        await this.minioService.removeObject(objectKey).catch((cleanupErr) => {
          this.logger.error(
            `Failed to clean up orphaned MinIO object ${objectKey}: ${
      ...
      await unlink(file.path).catch((cleanupErr) => {
        this.logger.warn(
          `Failed to remove temp upload file ${file.path}: ${(cleanupErr as Error).message}`,
        );
      });
    }
  }
  ...
  async getDownloadStream(
    ...
    await this.permissionsService.requireAccess(
      userId,
      userEmail,
      objectId,
